...
namespace gam300 {
	...
	// Gets relevant component based on mask
	Component *ECSManager::getComponentByMask(ComponentTypeID type_id, EntityID entity_id) {
		auto it = CM.m_component_arrays.find(type_id);      // friend access ok
		if(it == CM.m_component_arrays.end() || !it->second) return nullptr;
		return it->second->get_raw(entity_id);
	}

	// Gets relevant component based on mask
	Component const *ECSManager::getComponentByMask(ComponentTypeID type_id, EntityID entity_id) const {
		auto it = CM.m_component_arrays.find(type_id);
		if(it == CM.m_component_arrays.end() || !it->second) return nullptr;
		return it->second->get_raw(entity_id);
	}
