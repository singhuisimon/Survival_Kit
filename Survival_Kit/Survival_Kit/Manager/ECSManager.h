...
namespace gam300 {
	...
	public:
		...
		/*!***********************************************************************
		\brief
		Returns a mutable base-pointer to the component of type \p type_id that
		belongs to \p entity_id, or nullptr if the entity does not own that
		component or the type is unregistered.

		\param type_id
		Runtime component type identifier (bit/index into the component registry).

		\param entity_id
		Target entity ID.

		\return
		Pointer to the component as a \c Component*, or nullptr if absent.
		*************************************************************************/
		Component *getComponentByMask(ComponentTypeID type_id, EntityID entity_id);

		/*!***********************************************************************
		\brief
		Const-qualified overload that returns a read-only base-pointer to the
		component of type \p type_id on \p entity_id, or nullptr if absent.
		...
		\return
		Pointer to the component as a \c Component const*, or nullptr if absent.
		*************************************************************************/
		Component const *getComponentByMask(ComponentTypeID type_id, EntityID entity_id) const;
