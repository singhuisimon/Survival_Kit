...
//    return illumination;
    ...
    // Compute ambient light contribution
    vec3 ambient = light.La * material_Ka;

    // Compute diffuse light contribution
    float radiantEnergy = max(dot(normal, vectorL), 0.0);
    vec3 diffuse = light.Ld * material_Kd * radiantEnergy;

    // Compute specular light contribution
    vec3 specular = vec3(0.0);
    if (0.0 < radiantEnergy) {
        specular = light.Ls * material_Ks * pow(max(dot(normal, vectorH), 0.0), shininess);  
    }
...
//
//    vec3 illumination = ambient + diffuse + specular;
    FragColor = vec4(BlinnPhong(Position, Normal, light, V), 1.0);
